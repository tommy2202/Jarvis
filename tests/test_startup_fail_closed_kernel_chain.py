...
def _mk_audit_mgr(tmp_path, event_bus):
    ...
    mgr = AuditTimelineManager(cfg=cfg, logger=None, event_bus=event_bus, telemetry=None, ops_logger=None)
    mgr.start()
    return mgr
...
def test_kernel_chain_missing_capability_engine_blocks(tmp_path, monkeypatch):
    ...
    eb = EventBus(cfg=EventBusConfig(enabled=True, max_queue_size=200, worker_threads=2, overflow_policy=OverflowPolicy.DROP_OLDEST), logger=_L())
...
def test_kernel_chain_web_enabled_locked_secure_store_blocks(tmp_path, monkeypatch):
    import jarvis.core.startup.checks as checks
    ...
    try:
        ...
        assert ev is not None
        reason_codes = (ev.details or {}).get("reason_codes") or []
        assert "web.remote_control" in reason_codes
    finally:
        eb.shutdown(0.5)


def test_kernel_chain_discovery_import_attempt_blocks(tmp_path, monkeypatch):
    import jarvis.core.startup.checks as checks
    import jarvis.core.modules.discovery as discovery
    ...
    (mod_dir / "module.json").write_text(
        """
{"schema_version": 1, "module_id": "demo", "version": "0.1.0", "display_name": "Demo", "description": "", "entrypoint": "demo:register", "intents": [], "module_defaults": {"enabled_by_default": false, "admin_required_to_enable": false}}
""".strip()
        + "\n",
        encoding="utf-8",
    )
    (modules_root / "demo_mod.py").write_text("x = 1\n", encoding="utf-8")

    def _importing_list_files(module_dir: str):  # noqa: ANN001
        importlib.import_module("demo_mod")
        return []

    monkeypatch.setattr(discovery, "list_files_for_fingerprint", _importing_list_files)
    ...
    try:
        ...
        assert ev is not None
        reason_codes = (ev.details or {}).get("reason_codes") or []
        assert "modules.discovery_no_import" in reason_codes
    finally:
        eb.shutdown(0.5)
